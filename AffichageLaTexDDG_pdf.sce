function AffichageLaTexDDG(X,Q,G,H,e,M1,M2,M3,M4,Mu1,Mu2,Mu3,Mu4,sigma,Fisher1,Fisher2,tab,val_min,val_max);// Il faut appeler la fonctionner avec le paramètre "tab" en plus
...
mfprintf(fd,'Ceci est une partie du compte rendu du mini projet de modelisation statistique, celle-ci concerne les Donnees Discretes Groupees. Ce document fait suite a une demande realisee par monsieur Dariush Ghorbanzadeh, dans le cadre du projet de derniere annee du cycle ingenieur.');
mfprintf(fd,'\\\');
mfprintf(fd,'newparagraphe{}\n');
...
mfprintf(fd,'\\\includegraphics[scale=0.75]{figure2.pdf}\n');
